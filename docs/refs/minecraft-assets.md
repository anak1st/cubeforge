# Minecraft 官方资源:来源、授权与分析

> 版本 26.2。提取自 Mojang 官方客户端 `client.jar`(经 `scripts/fetch-mc-assets.sh` 从官方 CDN 获取,2026-08-31)。
> © Mojang Studios / Microsoft,保留所有权利:非开源、不可再分发,**仅限本机学习自用**。
> `temp/minecraft-assets/` 与 `public/textures/` 均已 gitignore——不入库、不进分发产物;新克隆请运行获取脚本。
